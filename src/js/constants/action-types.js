export const ADD_ARTICLE = "ADD_ARTICLE";
export const DELETE_ARTICLE = "DELETE_ARTICLE";

export const FETCH_ARTICLES = "FETCH_ARTICLES";
export const ARTICLES_FETCH_SUCCEEDED = "ARTICLES_FETCH_SUCCEEDED";
export const ARTICLES_FETCH_ERROR = "ARTICLES_FETCH_ERROR";

export const FETCH_USER = "FETCH_USER";
export const USER_FETCH_SUCCEEDED = "USER_FETCH_SUCCEEDED";
export const USER_FETCH_ERROR = "USER_FETCH_ERROR";

export const FETCH_TOKEN = "FETCH_TOKEN";
export const TOKEN_FETCH_SUCCEEDED = "TOKEN_FETCH_SUCCEEDED";
export const TOKEN_FETCH_ERROR = "TOKEN_FETCH_ERROR";

export const FETCH_GENRES = "FETCH_GENRES";
export const FETCH_GENRES_SUCCEEDED = "FETCH_GENRES_SUCCEEDED";
export const FETCH_GENRES_ERROR = "FETCH_GENRES_ERROR";

export const FETCH_GENRES_GAMES = "FETCH_GENRES_GAMES";
export const FETCH_GENRES_GAMES_SUCCEEDED = "FETCH_GENRES_GAMES_SUCCEEDED";
export const FETCH_GENRES_GAMES_ERROR = "FETCH_GENRES_GAMES_ERROR";

export const FETCH_GAMES = "FETCH_GAMES";
export const FETCH_GAMES_SUCCEEDED = "FETCH_GAMES_SUCCEEDED";
export const FETCH_GAMES_ERROR = "FETCH_GAMES_ERROR";